...
final class _PreviewPageState extends State<_PreviewPage> {
  ...
  static const Duration _postPopupDockDelay = Duration(milliseconds: 120);
              ...
              sliver: SliverList.list(
                children: <Widget>[
                  ...
                  const _SectionHeader(
                    ...
                    description:
                        'Compact mode rests as one full-size anchor plus one '
                        'smaller satellite when a second state is available. '
                        'Tap the dock to reveal the full chain, or long-press '
                        'the card to edit personal state. Real mutations wait '
                        'until the actions surface has fully closed.',
                  ),
                  ...
                  const _SectionHeader(
                    ...
                    description:
                        'Both examples contain the same personal state. Compact '
                        'mode rests as an anchor plus satellite; Permanent mode '
                        'keeps every active visible indicator fully expanded.',
                  ),
                  ...
                  const _SectionHeader(
                    ...
                    description:
                        'Anchor priority follows the canonical hierarchy: '
                        'Rating → Watchlist → Favorite → Collection. The '
                        'second active visible state becomes the satellite.',
                  ),
  ...
  Future<void> _applyPersonalState({
    ...
    Duration reactionDelay = Duration.zero,
  }) async {
    ...
    // The dock must first rebuild with the final application state so it can
    // snapshot the old and new structures before revealChanges() takes over.
    ...
    // Popup-originated edits get a short visual handoff after the route has
    // disappeared. Direct Workbench edits intentionally use Duration.zero.
    if (reactionDelay > Duration.zero) {
      await Future<void>.delayed(reactionDelay);
  ...
  void _setIndicatorVisibility(
    ...
    unawaited(_updateIndicatorVisibility(indicator, visible));
  }

  Future<void> _updateIndicatorVisibility(
    CinearaStatusDockIndicator indicator,
    bool visible,
  ) async {
    ...
    // Visibility controls are presentation preferences, not media-state
    // mutations. Compact previews use reveal(), never revealChanges().
    if (_mode == CinearaStatusDockMode.compact &&
        _visibleIndicators.isNotEmpty) {
      await _dockController.reveal();
    }
  }
  ...
  Future<void> _showQuickActions() async {
    bool draftFavorite = _favorite;
    bool draftCollection = _collection;
    bool draftWatchlist = _watchlist;
    double? draftRating = _personalRating;
    ...
    // The modal route has completed. Commit the final application state,
    // rebuild the dock, then give the poster a short 120 ms visual handoff
    // before the real mutation choreography begins.
    await _applyPersonalState(
      favorite: result.favorite,
      collection: result.collection,
      watchlist: result.watchlist,
      personalRating: result.personalRating,
      reactionDelay: _postPopupDockDelay,
    );
  }
  ...
  Widget build(BuildContext context) {
    return Wrap(
      ...
      children: <Widget>[
        _DockSpecimen(
          title: 'Rating anchor',
          subtitle: 'Rating → Watchlist satellite',
          index: 0,
          dock: CinearaStatusDock(
            labels: labels,
            mode: CinearaStatusDockMode.compact,
        ...
        _DockSpecimen(
          ...
          subtitle: 'No Rating → Favorite satellite',
          index: 1,
          dock: CinearaStatusDock(
            labels: labels,
            mode: CinearaStatusDockMode.compact,
        ...
        _DockSpecimen(
          ...
          subtitle: 'No Rating/Watchlist → Collection satellite',
          index: 2,
          dock: CinearaStatusDock(
            labels: labels,
            mode: CinearaStatusDockMode.compact,
        ...
        _DockSpecimen(
          ...
          subtitle: 'Only Collection active',
          index: 3,
          dock: CinearaStatusDock(
            labels: labels,
            mode: CinearaStatusDockMode.compact,
        ...
        _DockSpecimen(
          title: 'Hidden Rating',
          subtitle: 'Watchlist becomes first visible',
          index: 0,
          dock: CinearaStatusDock(
            labels: labels,
            mode: CinearaStatusDockMode.compact,
    ...
    return Container(
      ...
      child: const Column(
        ...
        children: <Widget>[
          ...
          _ChecklistItem(
            text:
                'Compact rest shows one full-size anchor and, when available, one smaller satellite.',
          ),
          _ChecklistItem(
            text:
                'Canonical priority is Rating → Watchlist → Favorite → Collection.',
          ),
          _ChecklistItem(
            text:
                'The compact resting satellite is the only deliberate badge overlap.',
          ),
          _ChecklistItem(
            text:
                'Full-size badges never overlap or pass through one another during animation.',
          ),
          _ChecklistItem(
            text:
                'Tap reveal separates the satellite first, then magnetically attaches hidden states into vacant slots.',
          ),
          _ChecklistItem(
            text:
                'Compact collapse retracts hidden presentation nodes before the second state settles back into the satellite role.',
          ),
          _ChecklistItem(
            text:
                'Every real addition in Compact expands the old chain before the new full-size state magnetically attaches.',
          ),
          _ChecklistItem(
            text:
                'Every real removal in Compact expands the old chain so the removed state is visibly detached before reflow.',
          ),
          _ChecklistItem(
            text:
                'Permanent mode uses the same magnetic attach/detach and reflow language without expand/collapse.',
          ),
          _ChecklistItem(
            text:
                'Long-press changes never animate underneath the actions sheet.',
          ),
          _ChecklistItem(
            text:
                'After a popup closes, the preview waits 120 ms before starting revealChanges().',
          ),
          _ChecklistItem(
            text:
                'Multiple changes share one structural mutation sequence rather than replaying the dock repeatedly.',
          ),
          _ChecklistItem(
            text:
                'Existing Rating value changes count numerically without revealing the full chain.',
          ),
          _ChecklistItem(
            text:
                'Visibility controls are presentation-only and use reveal(), not revealChanges().',
          ),
          _ChecklistItem(
            text:
                'An empty visible state leaves no line, node, handle, or reserved dock artifact.',
          ),
          _ChecklistItem(text: 'Grid mode keeps the dock inside the poster.'),
          _ChecklistItem(
            text:
                'List mode pins the dock to the trailing bottom of a fixed-height row slot.',
          ),
          _ChecklistItem(
            text:
                'Reduced Motion removes decorative movement without changing state behavior.',
          ),
          _ChecklistItem(
            text: 'RTL mirrors logical start/end attachment correctly.',
          ),
        ],
      ),
    );
